(ns benchling.main
  "Kotodama WASM entrypoint for the Benchling clean-room actor (L5) — Clojure port.
  ...
  facts (`benchling.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "benchling")
(def tier "L5")
...
(def entity-specs
  [{:entity "CustomEntity" :plural "customentities" :id-prefix "benchlin_cus"
    :fields [:apiURL :entityRegistryId :folderId :modifiedAt :name :registryId :url :webURL]
    :required [:apiURL :modifiedAt]
    :coerce {}                                          :refs {}}
   {:entity "DnaSequence"  :plural "dnasequences"  :id-prefix "benchlin_dna"
    :fields [:apiURL :bases :entityRegistryId :folderId :isCircular :length :modifiedAt :name :registryId :url :webURL]
    :required [:apiURL :bases]
    :coerce {:isCircular :bool :length :int}          :refs {}}
   {:entity "Entry"        :plural "entries"       :id-prefix "benchlin_ent"
    :fields [:apiURL :displayId :entryTemplateId :folderId :modifiedAt :name :webURL]
    :required [:apiURL :modifiedAt]
    :coerce {}                                          :refs {}}
   {:entity "Folder"       :plural "folders"       :id-prefix "benchlin_fol"
    :fields [:name :parentFolderId :projectId]         :required [:name]
    :coerce {}                                          :refs {}}
   {:entity "Project"      :plural "projects"      :id-prefix "benchlin_pro"
    :fields [:name]                                     :required [:name]
    :coerce {}                                          :refs {}}
   {:entity "Request"      :plural "requests"      :id-prefix "benchlin_req"
    :fields [:apiURL :displayId :projectId :requestStatus :scheduledOn :webURL]
    :required [:apiURL :requestStatus]
    :coerce {}                                          :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"benchling.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "benchling-compat" :tier tier :entities entities} 200])
...
(defn start! [] :benchling-compat/ready)
